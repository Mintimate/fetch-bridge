...
export const safeHeaderName = z
  ...
  .refine(
    ...
    "Cookie, Authorization and X-Forwarded-* cannot be configured",
  );
...
export const routeSchema = z.object({
  ...
  pathPrefix: z
    .string()
    .trim()
    .regex(/^\/[a-zA-Z0-9._-]*$/, "Use a single URL prefix such as /firefox"),
  targetPath: z
    .string()
    .trim()
    .regex(/^\/(?!\/)/, "Target must be an absolute path, not a URL")
    .refine(
      (path) =>
        !path.includes("..") && !path.includes("?") && !path.includes("#"),
      "Target may not contain traversal, query, or fragment",
    ),

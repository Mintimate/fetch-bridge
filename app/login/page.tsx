...
export default function LoginPage() {
  ...
  async function submit(formData: FormData) { setError(""); const result = await signIn("credentials", { email: formData.get("email"), password: formData.get("password"), redirect: false }); if (result?.error) setError("邮箱或密码不正确，或管理员凭据尚未配置。"); else window.location.assign("/admin"); }

...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (!source) return Response.json({ error: "Source not found" }, { status: 400 });
    const route = await prisma.route.create({ data: { ...parsed.data, description: parsed.data.description || null } });
